...
import { Calendar, Pencil, Trash2, CheckCircle2, AlertTriangle, Clock, Droplets } from "lucide-react";
...
const STATUS_CONFIG = {
  fresh:    { label: "Fresh",    className: "bg-tint-mint   text-foreground border-brand-mint/30",             icon: CheckCircle2 },
  soon:     { label: "Use soon", className: "bg-tint-sun    text-foreground border-brand-sun/40",              icon: Clock },
  expiring: { label: "Expiring", className: "bg-tint-coral  text-foreground border-brand-coral/40",            icon: AlertTriangle },
  expired:  { label: "Expired",  className: "bg-destructive text-destructive-foreground border-transparent",   icon: AlertTriangle },
};

import { useState } from "react";

export default function ProductCard({ product, onDelete, onLogUsage, onToggleProjectPan }: Props) {
  ...
  return (
    <Card className="group relative overflow-hidden bg-card border-2 border-border shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-1 rounded-2xl flex flex-col gap-0 p-0">
      ...
      <div className="p-5 space-y-3.5 flex-1 flex flex-col">
        ...
        {/* Type chip */}
        <Badge variant="secondary" className="font-medium rounded-full w-fit text-xs">
          {product.type}
        </Badge>
        ...
        <div className="space-y-1 text-sm border-t border-border pt-3">
          <div className="flex items-center justify-between">
            ...
            {effExp && (
              <span className="text-xs text-muted-foreground">{format(effExp, "MMM d")}</span>
            )}
          </div>
          {product.notes && (
        ...
        <div className="flex gap-2 pt-1 mt-auto">
          ...
          <Button
            size="sm"
            variant="ghost"
            aria-label={product.is_project_pan ? "Remove from Project Pan" : "Add to Project Pan"}
            title={product.is_project_pan ? "Remove from Project Pan" : "Add to Project Pan"}
            className={product.is_project_pan ? "text-brand-sky" : "text-muted-foreground hover:text-brand-sky"}
            onClick={() => onToggleProjectPan?.(product.id, !product.is_project_pan)}
          >
          ...
          <Dialog open={finishOpen} onOpenChange={setFinishOpen}>
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <Button onClick={() => { setFinishOpen(false); onLogUsage?.(product.id); }}>
                  Yes, I&apos;m done!
                </Button>
              </DialogFooter>
            </DialogContent>
          ...
          <Dialog>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  This will permanently delete &quot;{product.brand} &mdash; {product.name}&quot;.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <DialogClose asChild>
